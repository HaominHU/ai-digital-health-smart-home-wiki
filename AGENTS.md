...
- `sources/papers/monthly_pubmed/`: Example purpose-specific lane for monthly PubMed push papers before they are selected, prioritized, or integrated. Use provisional topic-lens subfolders: `adoption_preferences_and_equity/`, `ai_interaction_and_decision_support/`, `caregiving_support_systems/`, `smart_home_and_ambient_care/`, and `wearables_and_remote_monitoring/`. Assign by the paper's primary contribution, not every theme it touches. Keep screened but withdrawn papers under `deferred_or_out_of_scope/` rather than mixing them into active topic folders or deleting them. Revisit the taxonomy when future papers conflict with these categories.
